...
internal class GenreTabFragment :
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		view_pager.apply {
			adapter = this@GenreTabFragment.adapter
			onPageSelected { postIntent(GenreListIntent.ChangeCurrentGenre(it)) }
	...
	// https://medium.com/@omneyaosman/implement-viewpager2-as-a-recyclerview-7456803d102e
	private fun View.setParallaxTransformation(position: Float) {
		if (position < -1 || position > 1) return

		when ((this as ViewGroup).children.first().tag) {
